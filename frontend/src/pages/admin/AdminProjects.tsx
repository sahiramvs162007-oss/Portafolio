...
export default function AdminProjects() {
  ...
  const openModal = (project?: any) => {
    setCurrentProject(
      project || { titulo: "", descripcion: "", githubUrl: "", demoUrl: "" },
    );
    setIsModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (currentProject.githubUrl)
        formData.append("githubUrl", currentProject.githubUrl);
      if (currentProject.demoUrl)
        formData.append("demoUrl", currentProject.demoUrl);
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((proj) => (
          <div
            key={proj._id}
            className="bg-ink-900 border border-ink-800 rounded-xl p-4 flex flex-col"
          >
            {proj.imagenUrl && (
              <img
                src={proj.imagenUrl}
            ...
            <h3 className="font-bold text-lg text-ink-50 mb-2">
              {proj.titulo}
            </h3>
            <p className="text-sm text-ink-300 line-clamp-3 mb-4 flex-1">
              {proj.descripcion}
            </p>
